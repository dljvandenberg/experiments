...
Als ENGIE beheren we een installatie waar helaas een technische fout is ingeslopen. Het gevolg is dat de druk in de installatie sterk kan oplopen. Om te voorkomen dat er gevaarlijke situaties ontstaan, hebben de installatietechnici een snelle oplossing bedacht: ’s nachts zet iemand handmatig een klep open waardoor de overdruk kan ontsnappen. Een mooie tijdelijke oplossing, maar liever nog liggen ze ’s nachts gewoon te slapen. In een discussie over de mogelijke oorzaken, gaf een van de experts aan het gevoel te hebben dat de toename van de druk niet altijd hetzelfde patroon volgt. “Het lijkt wel alsof de installatie goede en slechte dagen heeft”. De vragen waar ze mee naar jou toe zijn gekomen zijn de volgende: [in het grijs staan de tips die je van een collega krijgt en we graag hebben dat je toepast] 

1.	Kun jij op basis van de data het gevoel van de expert bevestigen?  
a.	Pas een mixed effect model toe. Je kunt daarbij bijvoorbeeld de lmer function uit het R-pakket lme4 gebruiken (of een vergelijkbaar pakket indien je in Python werkt). De functie krijgt dan bijvoorbeeld de vorm: myModel <- lmer(druk ~ tijd + (tijd | groupID) , data = myData).
2.	Als het gevoel inderdaad bevestigd kan worden, kun je de verschillende dagen automatisch `	herkennen?  
a.	Gebruik de intercepten en/of slope (aan jou de keuze) uit vraag 1 als input voor een k-means om unsupervised een classificatie te maken. Sla deze op in een myKmeansModel object.  
b.	Plot een grafiek waarin je duidelijk ziet dat je de clustering goed hebt toegepast.  
...
a.	Schrijf een functie waarmee je op basis van een beperkte set aan datapunten de intercept en/of slope bepaalt.  
b.	K-means kent (in R) geen predict function, dus schrijf een functie die op basis van de intercept en/of slope en het myKmeansModel object de bijbehorende cluster bepaalt.  
...
predict_cluster <- function(coefficients, kmeansmodel){
  ...
  # Calculate distance to cluster_centers
...
# Test
#assertthat::assert_that(predicted_cluster == 1, msg = "Predicted cluster should be 1")
```
...
```{r}
factoextra::get_dist(cluster_centers, method = 'euclidean')
```
